...
val changeStartPagePatch = bytecodePatch(
    ...
    execute {
        val coldStartIntentMethod = coldStartIntentFingerprint.methodOrWarn(CHANGE_START_PAGE.title)
            ?: return@execute
        val coldStartUpMethod = coldStartUpFingerprint.methodOrWarn(CHANGE_START_PAGE.title)
            ?: return@execute

        coldStartIntentMethod.addInstruction(
            0,
            "invoke-static {p1}, $EXTENSION_CLASS_DESCRIPTOR->overrideIntent(Landroid/content/Intent;)V"
        )

        coldStartUpMethod.apply {
