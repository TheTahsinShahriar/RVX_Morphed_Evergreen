    ...
    execute {
        fun tryPatch(sectionName: String, block: () -> Unit): Boolean =
            try {
                ...
                printWarn("Skipping \"${NAVIGATION_BAR_COMPONENTS.title}\" section \"$sectionName\": ${exception.message}")
                false
            }
        /**
         * Enable custom navigation bar color
         */
        tryPatch("custom navigation bar color") {
         ...
         */
        tryPatch("hide navigation labels") {
         ...
         */
        tryPatch("hide navigation bar and buttons") {
