...
val seekbarComponentsPatch = bytecodePatch(
    ...
    execute {
        fun tryPatch(sectionName: String, block: () -> Unit): Boolean =
            try {
                block()
                true
            } catch (exception: PatchException) {
                printWarn("Skipping \"${SEEKBAR_COMPONENTS.title}\" section \"$sectionName\": ${exception.message}")
                false
            }
        ...
        tryPatch("enable seekbar tapping") {
        ...
        tryPatch("append time stamps information") {
        ...
        if (is_19_25_or_greater) {
            if (tryPatch("seekbar color accent and gradients") {
        ...
        tryPatch("watch history progress drawable resource") {
        ...
        if (is_19_25_or_greater) {
            ...
            tryPatch("splash seekbar color resources") {
                arrayOf(
