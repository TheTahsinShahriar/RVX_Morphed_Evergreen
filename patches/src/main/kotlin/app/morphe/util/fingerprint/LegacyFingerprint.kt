...
context(BytecodePatchContext)
internal fun Pair<String, Fingerprint>.matchOrWarn(featureName: String): Match? =
    matchOrNull().also {
        if (it == null) {
            printWarn("Skipping \"$featureName\": failed to resolve $first.")
        }
    }

context(BytecodePatchContext)
internal fun Pair<String, Fingerprint>.matchOrWarn(
    featureName: String,
    parentFingerprint: Pair<String, Fingerprint>
): Match? =
    matchOrNull(parentFingerprint).also {
        if (it == null) {
            printWarn("Skipping \"$featureName\": failed to resolve $first.")
        }
    }
...
context(BytecodePatchContext)
internal fun Pair<String, Fingerprint>.methodOrWarn(featureName: String): MutableMethod? =
    methodOrNull().also {
        if (it == null) {
            printWarn("Skipping \"$featureName\": failed to resolve $first.")
        }
...
context(BytecodePatchContext)
internal fun Pair<String, Fingerprint>.methodOrWarn(
    featureName: String,
    parentFingerprint: Pair<String, Fingerprint>
): MutableMethod? =
    matchOrNull(parentFingerprint)?.method.also {
        if (it == null) {
            printWarn("Skipping \"$featureName\": failed to resolve $first.")
        }
    }
